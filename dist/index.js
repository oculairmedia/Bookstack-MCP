...
function configureHttpTransportHealth(healthEndpoint) {
    configuredHealthEndpoint = healthEndpoint;
    if (httpTransportPatched) {
        return;
    }
    const prototype = HttpStreamTransport.prototype;
    ...
    prototype.start = async function patchedStart(...args) {
        ...
        server.on("request", (req, res) => {
            ...
            const healthPath = this.__healthEndpoint;
...
function extractPathname(url, host) {
    try {
...
function resolveHttpResponseMode() {
    const envMode = (process.env.HTTP_RESPONSE_MODE ?? "stream").toLowerCase();
    return HTTP_RESPONSE_MODES.has(envMode) ? envMode : "stream";
}
