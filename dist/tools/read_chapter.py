...
def read_chapter(id: int) -> str:
    """
    Retrieves details of a specific chapter in Bookstack.

    This function supports two usage scenarios:
      1) Retrieving a chapter's details
      2) Retrieving the tool's metadata by passing id="__tool_info__"

    Args:
        id (int): The ID of the chapter to retrieve
...
if __name__ == "__main__":
    print("Testing read_chapter tool...")
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = read_chapter("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example chapter retrieval (replace with your actual API key and a valid chapter ID)
    print("\nRetrieving test chapter (ID: 2)...")
    try:
        chapter_details = read_chapter(2)  # Assuming chapter ID 1 exists
        response = json.loads(chapter_details)
        if "error" in response:
            print(f"Error retrieving chapter: {response['error']}")
        else:
