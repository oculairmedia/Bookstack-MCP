...
class BookstackSearchTool extends BookstackTool {
    constructor() {
        super(...arguments);
        this.name = "bookstack_search";
        this.description = "Searches content across Bookstack with advanced query support";
        this.schema = schema;
    }
    async execute(input) {
        const queryParams = {
        ...
        return this.runRequest(async () => {
            const data = await this.getRequest("/api/search", { query: queryParams });
    ...
    extractSummary(item) {
        ...
        if (preview && typeof preview === "object" && !Array.isArray(preview)) {
            const content = this.asString(preview.content);
            if (content) {
