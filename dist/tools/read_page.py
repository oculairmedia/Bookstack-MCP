...
def read_page(id: int) -> str:
    """
    Retrieves details of a specific page in Bookstack.

    This function supports two usage scenarios:
      1) Retrieving a page's details
      2) Retrieving the tool's metadata by passing id="__tool_info__"

    Args:
        id (int): The ID of the page to retrieve
...
if __name__ == "__main__":
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = read_page("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example page retrieval
    print("\nRetrieving test page (ID: 3)...")  # Updated to ID 3
    try:
        page_details = read_page(3)
        response = json.loads(page_details)
        if "error" in response:
            print(f"Error retrieving page: {response['error']}")
        else:
            print("Page retrieved successfully!")
            print("\nPage Details:")
            print(f"Name: {response.get('name')}")
            print(f"Book ID: {response.get('book_id')}")
            print(f"Chapter ID: {response.get('chapter_id')}")
            print(f"\nMarkdown Content:")
