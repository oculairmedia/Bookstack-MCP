...
def delete_book(id: int) -> str:
    ...
    try:
        ...
        return json.dumps({"success": True, "message": f"Book with ID {id} deleted successfully"})
...
if __name__ == "__main__":
    print("Testing delete_book tool...")
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = delete_book("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example book deletion
    print("\nDeleting test book (ID: 7)...")
    try:
        result = delete_book(7)  # Delete our test book
        response = json.loads(result)
        if "error" in response:
            print(f"Error deleting book: {response['error']}")
        else:
