...
def read_bookshelf(id: int) -> str:
    """
    Retrieves details of a specific bookshelf in Bookstack.

    This function supports two usage scenarios:
      1) Retrieving a bookshelf's details
      2) Retrieving the tool's metadata by passing id="__tool_info__"

    Args:
        id (int): The ID of the bookshelf to retrieve
...
if __name__ == "__main__":
    print("Testing read_bookshelf tool...")
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = read_bookshelf("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example bookshelf retrieval
    print("\nRetrieving test bookshelf (ID: 3)...")
    try:
        bookshelf_details = read_bookshelf(3)  # Assuming bookshelf ID 1 exists
        response = json.loads(bookshelf_details)
        if "error" in response:
            print(f"Error retrieving bookshelf: {response['error']}")
        else:
            print("Bookshelf retrieved successfully!")
            print("\nBookshelf Details:")
