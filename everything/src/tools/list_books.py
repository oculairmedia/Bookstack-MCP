...
def list_books(offset: int = 0, count: int = 100) -> str:
    """
    Lists all books in Bookstack with pagination support.
...
if __name__ == "__main__":
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = list_books("__tool_info__")
    ...
    try:
        ...
        response = json.loads(books_list)
        if "error" in response:
            print(f"Error retrieving books: {response['error']}")
        else:
