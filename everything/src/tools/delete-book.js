/**
 * Tool handler for deleting a book from Bookstack
 */
export async function handleDeleteBook(server, args) {
    try {
        // Validate arguments
        ...
        console.log(`Using Bookstack API at ${baseUrl} to delete book with ID ${args.id}`);
        ...
        try {
            ...
            return {
                content: [{
                    ...
                    text: JSON.stringify({
                        success: true,
                        message: `Book with ID ${args.id} deleted successfully`
                    }, null, 2),
                }],
            };
        } catch (error) {
...
/**
 * Tool definition for delete_book
 */
export const deleteBookToolDefinition = {
    name: 'delete_book',
