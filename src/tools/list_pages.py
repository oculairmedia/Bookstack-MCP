...
def list_pages(offset: int = 0, count: int = 100) -> str:
    """
    Lists all pages in Bookstack with pagination support.
...
if __name__ == "__main__":
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = list_pages("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example list retrieval
    print("\nRetrieving list of pages...")
    try:
        pages_list = list_pages()
        response = json.loads(pages_list)
        if "error" in response:
            print(f"Error retrieving pages: {response['error']}")
        else:
