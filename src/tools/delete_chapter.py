...
def delete_chapter(id: int) -> str:
    """
    Deletes a chapter from Bookstack.

    This function supports two usage scenarios:
      1) Deleting a chapter
    ...
    try:
        ...
        return json.dumps({"success": True, "message": f"Chapter with ID {id} deleted successfully"})
...
if __name__ == "__main__":
    print("Testing delete_chapter tool...")
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = delete_chapter("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example chapter deletion
    print("\nDeleting test chapter (ID: 4)...")
    try:
        result = delete_chapter(4)  # Delete our test chapter
        response = json.loads(result)
        if "error" in response:
            print(f"Error deleting chapter: {response['error']}")
        else:
