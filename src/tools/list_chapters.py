...
def list_chapters(offset: int = 0, count: int = 100) -> str:
    """
    Lists all chapters in Bookstack with pagination support.

    This function supports two usage scenarios:
      1) Listing all chapters
...
if __name__ == "__main__":
    print("Testing list_chapters tool...")
    ...
    # Retrieve tool info
    print("\nTool Information:")
    tool_info = list_chapters("__tool_info__")
    print(json.dumps(json.loads(tool_info), indent=4))

    # Example list retrieval
    print("\nRetrieving list of chapters...")
    try:
        chapters_list = list_chapters()
        response = json.loads(chapters_list)
        if "error" in response:
            print(f"Error retrieving chapters: {response['error']}")
        else:
